...
# encoding: utf-8
'''
DARNN+self-attention 中attention中替换为resdual的selfattention 带dropout 
并且使用独立selfattention 带dropout
最后out3 使用resdual结构
加入relu
'''
import sys
...
class Encoder(nn.Module):
    ...
    def forward(self, X):
        ...
        X = self.scale*self.SelfAttention(X.permute(0,2,1)).permute(0,2,1)+X
...
class Decoder(nn.Module):
    ...
    def forward(self, X_encoded, y_prev):
        ...
        X_encoded = self.scale*self.SelfAttention(X_encoded)+X_encoded
