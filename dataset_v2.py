...
def stock_sample(input_):
    ...
    if df.iloc[iloc+target-1,:]['close'] > (1+alpha)*df.loc[d, 'close']:
        
        t = 2
    elif df.iloc[iloc+target-1,:]['close'] < (1-alpha)*df.loc[d, 'close']:
